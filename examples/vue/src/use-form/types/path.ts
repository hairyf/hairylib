 ...
 * See {@link Path}
 */
export type ValuePath<Values extends StructValues> = Path<Values>
/**
 * Helper type for recursively constructing paths through a type.
 * This actually constructs the strings and recurses into nested
 * object types.
 ...
 * See {@link ArrayPath}
 */
export type FieldArrayPath<Values extends StructValues> = ArrayPath<Values>
/**
 ...
 * See {@link PathValue}
 */
export type ValuePathValue<Values extends StructValues, TValuePath extends ValuePath<Values>> = PathValue<Values, TValuePath>
/**
 * See {@link PathValue}
 */
export type FieldArrayPathValue<Values extends StructValues, TFieldArrayPath extends FieldArrayPath<Values>> = PathValue<Values, TFieldArrayPath>
/**
 * Type to evaluate the type which the given paths point to.
 * @typeParam Fields - field values which are indexed by the paths
 * @typeParam TPath        - paths into the deeply nested field values
 * @example
 * ```
 * ValuePathValues<{foo: {bar: string}}, ['foo', 'foo.bar']>
 *   = [{bar: string}, string]
 * ```
 */
export type ValuePathValues<Values extends StructValues, TPath extends ValuePath<Values>[] | readonly ValuePath<Values>[]> = {} & {
  [K in keyof TPath]: ValuePathValue<Values, TPath[K] & ValuePath<Values>>;
}
/**
 ...
 * @example
 * ```typescript
 * ValuePathByValue<{foo: {bar: number}, baz: number, bar: string}, number>
 *   = 'foo.bar' | 'baz'
 * ```
 */
export type ValuePathByValue<Values extends StructValues, TValue> = {
  [Key in ValuePath<Values>]: ValuePathValue<Values, Key> extends TValue ? Key : never;
}[ValuePath<Values>]
/**
 ...
 * ```
 */
export type FieldArrayPathByValue<Values extends StructValues, TValue> = {
  [Key in FieldArrayPath<Values>]: FieldArrayPathValue<Values, Key> extends TValue ? Key : never;
}[FieldArrayPath<Values>]
export {}
