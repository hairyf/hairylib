...
export const propKey = '__hairyPropsReservedKey'
...
export type BuildPropOption<T, D extends BuildPropType<T, V, C>, R, V, C> = {
  ...
  validator?: ((value: any) => value is C) | ((value: any) => boolean)
}
...
>(option: BuildPropOption<T, D, R, V, C>, key?: string): BuildPropReturn<T, D, R, V, C> {
  ...
  const prop: any = {
    type: isObject(type) && Object.getOwnPropertySymbols(type).includes(wrapperKey) ? (type as any)[wrapperKey] : type,
    required: !!required,
...
export function definePropType<T>(value: any) {
  return { [wrapperKey]: value } as PropWrapper<T>
}
