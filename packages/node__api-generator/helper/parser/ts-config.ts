...
const helperOutput = (config: OpenAPIBuildConfiguration): BuildOutput[] => {
  const output = (function () {
    if (typeof config.output === 'string') return { main: config.output, cwd: '', type: '' }
  ...
  let type = ''
  if (output?.type === true || typeof output?.type === 'undefined') type = api.replace(/\.ts|\.js/g, '.type.ts')
  if (typeof output?.type === 'string') type = output?.type
